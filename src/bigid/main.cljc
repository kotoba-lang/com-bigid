(ns bigid.main
  "Kotodama WASM entrypoint for the Bigid clean-room actor (L4) — Clojure port.
  ...
  facts (`bigid.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"bigid.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bigid-compat" :tier tier :entities entities} 200])
